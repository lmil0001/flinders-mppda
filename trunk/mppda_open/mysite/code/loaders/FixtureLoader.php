 ...
 *
 * To prepare fixtures for loading, add the following to your _config.php. Note
 * that 'type' and 'filter' are used to search for an existing record so that
 * the fixture isn't bootstrapped multiple times.
 *
		...
		foreach (self::$preload_fixtures as $desc) {
			...
			if (file_exists(Director::baseFolder().'/'.$fixtureFile)) {
				...
				if (!$existing) {
					...
					if (array_key_exists('callback', $desc)) {
						call_user_func($desc['callback'], $fixture);
					}
