...
class MetaManagerDataDecorator extends SiteTreeDecorator { 
	...
	public function updateCMSFields(FieldSet &$fields) {
		...
		if(self::$show_checkbox == 1){
			$fields->addFieldToTab(self::$checkbox_tab, new CheckboxField('GenerateMetaData', _t('MetaManager.GENERATEMETADATA','Generate Metadata automatically from the page content') . self::updatedFields() , $this->owner->GenerateMetaData), self::$checkbox_pos);
		} else {
			$fields->addFieldToTab(self::$checkbox_tab, new HiddenField('GenerateMetaData', _t('MetaManager.GENERATEMETADATA','Generate Metadata automatically from the page content'), $this->owner->GenerateMetaData), self::$checkbox_pos);
		}
